...
# Process names that might contain a number for ordering
# for app_name in app_names:
#     tmp = app_name.split('-')
    
#    examples.append(tmp[1] if tmp[0].isdigit() else app_name)
...
def render():
    ...
    for i in range(max_icons):
        ...
        # Remove digit from label. A digit can be prepended to the app if ordering is required.
        tmp = label.split('-')
        clean_label = tmp[1] if tmp[0].isdigit() else tmp[0]
        
        icon_label = clean_label.replace("_", "-")
        icon = f"{APP_DIR}/icon-{icon_label}.jpg"
        label = clean_label.replace("_", " ")
        jpeg.open_file(icon)
        jpeg.decode(x - 26, 30)
        display.set_pen(0)
